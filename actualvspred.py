...
for species_file in species_files:
    ...
    # Exclude the last three non-sequential years
    last_years_to_exclude = sorted(grouped_data['year'].unique())[-3:]
    data_to_fit = grouped_data[~grouped_data['year'].isin(last_years_to_exclude)]

    # Store data for the species
    species_data[species_name] = {
        'N': len(data_to_fit),
        'T': max(data_to_fit['year']),
        'year': data_to_fit['year'].values,
        'count': data_to_fit['occurrences'].values,
        'exclude': last_years_to_exclude,
        'full_year': grouped_data['year'].values, 
        'full_count': grouped_data['occurrences'].values
    }
...
for i, (species_name, data) in enumerate(species_data.items()):
    ...
    # Fit the Stan model using data up to the last three non-sequential years
    fit = model.sample(
        num_chains=4,
        num_samples=2000
    )
    ...
    # Plot occurrences per year for all years
    axes[i].plot(data['full_year'], data['full_count'], label='Observed', marker='o', linestyle='-', color='blue')

    fut_pred = []

    # Highlight predicted occurrences for the last three non-sequential years
    for future_year in data['exclude']:
    ...
    axes[i].plot(data['exclude'], fut_pred, marker='o', markersize=8, label='Predicted', color='green')
    axes[i].fill_between(data['exclude'], prediction_interval[0], prediction_interval[1], color='green', alpha=0.3)
